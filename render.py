...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, indices=None, obj_id=None):
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, indices=None, obj_id=None):
    with torch.no_grad():
        ...
        if not skip_train:
             render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, indices=indices, obj_id=obj_id)

        if not skip_test:
             render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, indices=indices, obj_id=obj_id)
