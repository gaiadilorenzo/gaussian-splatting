...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, indices: torch.Tensor = None):
    ...
    if pipe.compute_cov3D_python:
        cov3D_precomp = pc.get_covariance(scaling_modifier)[indices] if indices is not None else pc.get_covariance(scaling_modifier)
    else:
    ...
    if override_color is None:
        if pipe.convert_SHs_python:
            shs_view = pc.get_features[indices].transpose(1, 2).view(-1, 3, (pc.max_sh_degree+1)**2) if indices is not None else pc.get_features.transpose(1, 2).view(-1, 3, (pc.max_sh_degree+1)**2)
            dir_pp = (pc.get_xyz[indices] - viewpoint_camera.camera_center.repeat(pc.get_features.shape[0], 1)) if indices is not None else (pc.get_xyz - viewpoint_camera.camera_center.repeat(pc.get_features.shape[0], 1))
