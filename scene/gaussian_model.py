...
class GaussianModel:
    ...
    def create_from_gaussians(self, gaussians):
        self._xyz = nn.Parameter(gaussians._xyz.detach().cpu().requires_grad_(True).cuda())
        self._features_dc = nn.Parameter(
            gaussians._features_dc.detach().cpu().contiguous().requires_grad_(True).cuda()
        )
        self._features_rest = (
            nn.Parameter(gaussians._features_rest.detach().cpu().requires_grad_(True))
            if gaussians._features_rest is not None
            else torch.zeros(
                (self._xyz.shape[0], 3, (3 + 1) ** 2 - 3),
            ).transpose(1, 2)
            .contiguous()
            .cuda()
        )
        self._scaling = nn.Parameter(
            gaussians._scaling.detach().cpu().requires_grad_(True).cuda()
        )
        self._rotation = nn.Parameter(
            gaussians._rotation.detach().cpu().requires_grad_(True).cuda()
        )
        self._opacity = nn.Parameter(
            gaussians._opacity.detach().cpu().requires_grad_(True).cuda()
        )
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        features = (
            torch.zeros((fused_color.shape[0], 3, (3 + 1) ** 2))
            .float()
            .cuda()
        )
    ...
    def to_pt(self):
        ...
        if f_rest is None or f_rest.numel() == 0:
            f_rest = torch.zeros(
        ...
        splats = {
            ...
            "sh0": sh0.clone()
            .detach()
            .float(),  # Keep sh0 to be able to save the model
            "shN": f_rest.clone()
            .detach()
